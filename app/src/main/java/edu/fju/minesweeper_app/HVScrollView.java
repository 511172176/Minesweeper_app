...
/**
 * 類別功能描述：
 * 具有彈性效果的全方向ScrollView,參考ScrollView與HorizontalScrollView源碼
 *
 */
public class HVScrollView extends FrameLayout {
    ...
    /**
     * 設定scroller是否可以滑動內容當觸控螢幕事件在chileview之外 default:false
    ...
    public void computeScroll() {
        if (mScroller.computeScrollOffset()) {
            ...
            if (getChildCount() > 0) {
                ...
                super.scrollTo(x, y);
                // getHeight()- child.getHeight()=y ->底部 y=0 ->頂端
                // 慣性強度 mScroller.getDuration()
            }
            awakenScrollBars();
